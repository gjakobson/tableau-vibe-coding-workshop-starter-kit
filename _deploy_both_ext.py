...
COMPONENTS = ["salesCloudFunnel", "salesCloudTreemap"]
...
with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
    for name in COMPONENTS:
        lwc_dir = Path("lwc") / name
        for ext in (".js", ".html", ".css", ".js-meta.xml"):
            zf.writestr(f"lwc/{name}/{name}{ext}", (lwc_dir / f"{name}{ext}").read_text())

    zf.writestr("staticresources/d3.resource", d3_js)
    zf.writestr("staticresources/d3.resource-meta.xml", D3_META)

    members = "\n".join(f"  <types><members>{n}</members><name>LightningComponentBundle</name></types>" for n in COMPONENTS)
...
if not match:
    print("Deploy failed to start:", r.text[:400]); raise SystemExit(1)
...
if state == "Succeeded":
    print(f"  ✅ Deployed: {', '.join(COMPONENTS)}")
else:
    for f in (status.get("details", {}).get("componentFailures") or []):
        print(f"  ❌ {f.get('fileName')} — {f.get('problem')}")
    raise SystemExit(1)
